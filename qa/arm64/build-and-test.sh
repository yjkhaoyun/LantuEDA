...
IMAGE_NAME="kicad-arm64-qa"
CONTAINER_BUILD_DIR="/src/build/arm64"

# QA test names matching CI configuration (skip python/cli which need wxPython)
...
usage() {
    ...
    echo ""
    echo "Commands:"
    echo "  build-image   Build the ARM64 Docker image"
...
docker_run() {
    docker run --rm --platform linux/arm64 \
        -v "${SRC_DIR}:/src:rw" \
        -w /src \
        -e CCACHE_DIR=/src/build/arm64-ccache \
        "$IMAGE_NAME" \
        bash -c "$1"
}
...
cmd_build() {
    echo "=== Configuring and building QA tests for ARM64 ==="
    docker_run "
        ...
        echo 'KICAD_BUILD_ARCH detection:' &&
        grep -i 'KICAD_BUILD_ARCH' CMakeCache.txt || true &&
        ninja 2>&1 | tee build_log.txt
    "
}

cmd_test() {
    echo "=== Running QA tests for ARM64 ==="
    local failed=()
...
cmd_shell() {
    echo "=== Opening shell in ARM64 container ==="
